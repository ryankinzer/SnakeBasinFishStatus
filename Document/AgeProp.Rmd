...
We then imposed some hierarchy by assuming that the vector of age proportions for each branch, $\pi_{pop_i}$, is drawn from a multivariate logistic normal distribution, with mean vector $\mu$ and covariance matrix $\Sigma$. To account for the potential differences in age proportions between A-run and B-run populations, we used 2 different $\mu$ vectors, one for each type of population.
...
```{r hierarchical_v2, eval = T, echo = T}
modelNm = '../ModelFiles/AgePropJAGS_hier.txt'
...
tagSummDir = '../data/LifeHistoryData/'
