...
ggsave(paste0('Figures/LGR_win_abund_',spp,'.png'),
       lgr_win_fig,
       width = 11,
       height = 8)
...
allAbundDf = as.list(yr_range) %>%
  ...
  map_df(.id = 'spawn_year',
         .f = function(x) {
           ...
           N_pop_summ %>%
           left_join(trt_df %>%
                       select(MPG, POP_NAME, TRT), by = 'TRT') %>%
             filter(median != 0)
         })
...
ts_names <- allAbundDf %>%
  left_join(valid_est %>%
              select(spawn_yr, species, TRT, valid_est), by = c('spawn_yr', 'species', 'TRT')) %>%
  filter(valid_est == 1) %>%
  group_by(MPG, TRT) %>%
  top_n(-1,spawn_yr) %>%
  select(spawn_yr, MPG, TRT, median)# %>%
#filter(!grepl('GRLOS/GRMIN',TRT))
...
pop_N_MPG <- pop_N +
  facet_wrap(~MPG, labeller = label_wrap_gen(width = 30), scales = 'free_y', ncol = 2) + #space = 'free_y', 
  ggrepel::geom_label_repel(data = ts_names, aes(x = spawn_yr, y = median, label = TRT), size = 2) #, position = position_nudge(y = 200, x=.25)) +
...
ggsave(paste0('Figures/PopAbund_MGP_',spp,'.png'),
       pop_N_MPG,
       width = 8,
       height = 10)

ggsave(paste0('Figures/PopAbund_POP_',spp,'.png'),
       pop_N_POP,
       width = 8,
       height = 10)
...
#  facet_grid(spawn_year ~., scales = "free_y", space = 'free_y', labeller = label_wrap_gen(width = 10)) +
  facet_wrap(~spawn_year, scales = 'free_y',ncol =2, dir = 'v') +
  labs(x = '',
       y = 'Female Proportion',
...
sex_Pred
# library(patchwork)
# sex_Pred <- a_sex_Pred + b_sex_Pred & theme(legend.position = 'bottom')
# 
# sex_Pred <- sex_Pred + plot_layout(guides = 'collect')

ggsave(paste0('Figures/SexProp',spp,'.png'),
       sex_Pred,
       width = 7,
       height = 9)

# get df of all years and pops
tmp <- expand_grid(TRT = unique(allSexDf$TRT),
                  spawn_year = as.character(yr_range)) %>%
  left_join(allSexDf %>% 
              select(TRT, MPG) %>%
              distinct()) %>%
  mutate(`50%` = NA,
...
na_yrs <- anti_join(tmp, allSexDf, by = c('spawn_year', 'TRT'))

sex_Pred2 <- allSexDf %>%
  select(TRT, spawn_year, MPG, `50%`, `2.5%`, `97.5%`) %>%
  bind_rows(na_yrs) %>%
  #mutate(MPG = factor(MPG, levels = c(mpg_ord,'Overall'))) %>%
  filter(TRT != 'Overall') %>%
  #filter(spawn_year == '2018') %>%
  ...
  guides(colour = guide_legend(nrow=3)) +
  #  facet_grid(spawn_year ~., scales = "free_y", space = 'free_y', labeller = label_wrap_gen(width = 10)) +
  #facet_wrap(~TRT, scales = 'free_y',ncol =2) +
  labs(x = 'Spawn Year',
       y = 'Female Proportion',
       #title = paste0('Natural-Origin ',spp, ' Female Proportions'),
       color = 'Major Population Group')
...
ggsave(paste0('Figures/SexProp2',spp,'.png'),
       sex_Pred2,
       width = 7,
       height = 5)
# Age ------------------------------------------------------------------------------
...
allAgeDf <- allAgeDf %>%
mutate(Run = if_else(TRT %in% c('CRLOC-s',
                                'CRLOL-s',
                                'CRSEL-s',
                                'CRSFC-s',
                                'MFBIG-s',
...
ageProp2_p = allAgeDf %>%
  select(spawn_year, MPG, TRT, Run, age, mean) %>%
  distinct() %>%
  ggplot(aes(x = spawn_year,
             y = mean,
             fill = as.factor(age))) +
...
# get df of all years and pops
tmp <- expand_grid(TRT = unique(allAgeDf$TRT),
                   age = unique(allAgeDf$age),
                   spawn_year = as.character(yr_range)) %>%
  left_join(allAgeDf %>% 
              select(TRT, MPG, Run) %>%
              distinct()) %>%
...
na_yrs <- anti_join(tmp, allAgeDf, by = c('spawn_year', 'TRT'))

ageProp3 <- allAgeDf %>%
  select(TRT, age, spawn_year, MPG, Run, `50%`, `2.5%`, `97.5%`) %>%
  bind_rows(na_yrs) %>%
  filter(age %in% 3:5) %>%
  ggplot(aes(x = spawn_year,
             y = `50%`,
...
ggsave(paste0('Figures/ageProp3',spp,'.png'),
       ageProp3,
       width = 7,
       height = 7)
